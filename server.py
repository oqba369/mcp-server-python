...
def video_download_my_video(video_id: str) -> dict:
    ...
    ydl_opts = {'outtmpl': outtmpl, 'format': 'bv*+ba/b', 'merge_output_format': 'mp4', 'ffmpeg_location': FFMPEG_EXE, 'quiet': True, 'no_warnings': True, 'noplaylist': True}
    url = f'https://www.youtube.com/watch?v={video_id}'
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            info = ydl.extract_info(url, download=True)
        candidates = sorted(MEDIA_DIR.glob(base.name + '.*'), key=lambda p: p.stat().st_mtime, reverse=True)
        if not candidates:
            raise RuntimeError('yt-dlp finished but no output file was found.')
        path = candidates[0]
        published = _publish_media(path)
        published.update({'video_id': video_id, 'title': title, 'source_watch_url': url, 'duration_seconds': info.get('duration')})
        return published
    except Exception as exc:
        raise RuntimeError('Could not download this owned YouTube video. If it is private/restricted, use the original source file from Drive/object storage instead. Details: ' + str(exc)) from exc
...
async def health(request: Request) -> Response:
    return JSONResponse({'status': 'ok', 'service': 'youtube-mcp', 'youtube_client_configured': bool(YOUTUBE_CLIENT_ID and YOUTUBE_CLIENT_SECRET and YOUTUBE_REDIRECT_URI), 'youtube_authorized': bool(YOUTUBE_REFRESH_TOKEN), 'ffmpeg': FFMPEG_EXE})
